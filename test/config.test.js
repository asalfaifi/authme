...
test('development configuration supplies safe defaults and freezes top-level collections', () => {
  ...
  assert.deepEqual(config.resourceServersByRealm, { master: [], engineering: [] });
  ...
  assert.equal(config.webauthnChallengeTtl, 300);
...
test('production configuration normalizes its URL and maps clients by realm', () => {
  ...
  const config = loadConfig(productionEnvironment({
    ...
    AUTHME_WEBAUTHN_CHALLENGE_TTL_SECONDS: '180',
  ...
  assert.deepEqual(config.resourceServersByRealm.master, [{
    audience: 'https://api.example.test/orders',
    scopes: ['orders.read', 'roles', 'groups'],
    authorizedClientIds: ['console'],
    introspectionClientIds: ['orders-api'],
  ...
  assert.equal(Object.isFrozen(config.resourceServersByRealm.master[0]), true);
  assert.equal(config.accessTokenTtl, 600);
  assert.equal(config.authorizationCodeTtl, 90);
  assert.equal(config.sessionTtl, 7200);
  assert.equal(config.webauthnChallengeTtl, 180);
...
test('realm, client, key, URL, and TTL validation fail closed', () => {
  ...
  assert.throws(
    () => loadConfig({ AUTHME_DEV_MODE: 'true', AUTHME_RESOURCE_SERVERS_JSON: '{not-json' }),
    /must contain valid JSON/,
  );
  assert.throws(
    () => loadConfig({
      AUTHME_DEV_MODE: 'true',
      AUTHME_RESOURCE_SERVERS_JSON: JSON.stringify({ master: [{
        audience: 'http://api.example.test', scopes: ['read'], authorized_client_ids: ['client'],
      }] }),
    }),
    /must use HTTPS/,
  );
  assert.throws(
    () => loadConfig({
      AUTHME_DEV_MODE: 'true',
      AUTHME_RESOURCE_SERVERS_JSON: JSON.stringify({ master: [{
        audience: 'http://127.0.0.1:4000/api', scopes: ['bad scope'], authorized_client_ids: ['client'],
      }] }),
    }),
    /invalid OAuth scope/,
  );
  ...
  assert.throws(
    () => loadConfig({ AUTHME_DEV_MODE: 'true', AUTHME_WEBAUTHN_CHALLENGE_TTL_SECONDS: '301' }),
  );
});
